...
var logger = log4js.getLogger("lib/xa-transaction-resource.js");


var CreateMode = zookeeper.CreateMode;
function XATransactionResource(workerScheduleName, distributedTransactionId, workerId, isFirst, isLast, executable){
	var that = this;
    ...
    that.tasksGetChildrenCallback = function(error, children, stats){
        ...
        if(error){
            ...
            logger.info('[' + that.workerId + '] Nuevas asignaciones obtenidas en path: ' + path);
        ...
        that.zk.getData(
            ...
            function(error, data, stat){
                if(error){
                    ...
                    result.then(function(dataResult){
                        logger.debug('[' + that.workerId + '] Resultado de ejecucion sobre tarea obtenido: ' + taskPath);
                        var clientId = dataResult[ASSIGN_METADATA_NODES.XA_ASSIGN_METADATA_NODE][ASSIGN_METADATA_NODES.CLIENT_ID_CHILD];
                        ...
                        if(that.isLast){
                            ...
                            that.statusCreate(resultPath, JSON.stringify(dataResult));
                        }
                        ...
                        if(!that.isLast){
                            ...
                            that.statusCreate(statusPath, JSON.stringify(dataResult));
                        }
                        ...
                        logger.debug('Dice que elimine asignacion: ' + assignPath);
                        that.assignDelete(assignPath);
                    }, function(err){
                        logger.error('Error al executar operacion sobre data');
                    });
    ...
    this.init = function(callback){
        logger.info('[' + that.workerId + '] Iniciaindo XA Transaction Resource');
        async.series({
...
module.exports = XATransactionResource;