...
function DistributedTransactionWorker(workerScheduleName, distributedTransactionId, workerId, isFirst, isLast, executable){
                ...
                that.zk.create(path, new Buffer(''), CreateMode.PERSISTENT, function (error, createdPath) {
                    if(error){
                        ...
                        case ZK_ERRORS.NODEEXISTS:
                            logger.warn("Ya existe al znode de namespace de worker. " + path);
                            serieCallback(null, path);
                            break;
                        default:
                            logger.error("Error al crear znode de namespace de worker. " + path);
            ...
            assignNamespace: function(serieCallback){
                ...
                that.zk.create(path, new Buffer(''), CreateMode.PERSISTENT, function (error, createdPath) {
                    if(error){
                        switch(error.code){
                    case ZK_ERRORS.NODEEXISTS:
                            logger.warn("Ya existe al namespace de asignaciones. " + path);
                            serieCallback(null, path);
                            break;
                        default:
                            logger.error("Error al crear namespace de asignaciones. " + path);
                            serieCallback(error, null);
                            break;
                        }
                    }else{
            ...
            statusNamespace: function(serieCallback){
                ...
                that.zk.create(path, new Buffer(''), CreateMode.PERSISTENT, function (error, createdPath) {
                    if(error){
                        switch(error.code){
                    case ZK_ERRORS.NODEEXISTS:
                            logger.warn("Ya existe al namespace de status. " + path);
                            serieCallback(null, path);
                            break;
                        default:
                            logger.error("Error al crear namespace de status. " + path);
                            serieCallback(error, null);
                            break;
                        }
                    }else{
	...
	that.createAssignNode = function(){
        ...
        that.zk.create(path, new Buffer(''), CreateMode.PERSISTENT, function (error, createdPath) {
            if(error){
                switch(error.code){
            	case ZK_ERRORS.NODEEXISTS:
                    logger.warn("Ya existe al namespace de asignacion. " + path);
                    break;
                ...
                logger.info('Namespace de asignaciones: %s creado.', createdPath);
            }
        });	
	};
	...
	that.register = function(){
		...
		that.zk.create(path, new Buffer('Idle'), CreateMode.EPHEMERAL, function (error, createdPath) {
            if(error){
                switch(error.code){
            	case ZK_ERRORS.NODEEXISTS:
                    logger.warn("El worker ya existe: " + path);
                    break;
                case ZK_ERRORS.CONNECTIONLOSS:
                	that.register();
                	break;
                default:
                    logger.error("Error al registrar worker: " + path);
                    serieCallback(error, null);
                    break;
                }
            }else{
                logger.info('Worker registrado exitosamente: ', createdPath);
            }
        });
	};
	...
	that.getTasks = function(){
		...
		that.zk.getChildren(
            ...
            // Watcher
			function(event){
				logger.debug('Watcher: nuevas tareas');
				that.getTasks();
			},
    ...
    that.tasksGetChildrenCallback = function(error, children, stats){
        ...
        if(error){
            ...
            case ZK_ERRORS.CONNECTIONLOSS:
                ...
                break;
            default:
                logger.error("Error al obtener asignaciones: " + path);
                break;
            }
        }else{
            logger.debug('Nuevas asignaciones obtenidas en path: ' + path);
            ...
            function(error, data, stat){
                if(error){
                    ...
                    case ZK_ERRORS.CONNECTIONLOSS:
                        ...
                        break;
                    default:
                        logger.error("Error al obtener datos de tarea: " + taskPath);
                        break;
                    }
                }else{
                    logger.info('Datos de tarea obtenidos exitosamente: ', taskPath);
                    ...
                    logger.debug('Resultado de ejecucion:');
                    console.dir(result);
        ...
        that.zk.create(path,
            ...
            function(error, path){
                if (error) {
                    switch(error.code){
                    case ZK_ERRORS.CONNECTIONLOSS:
                        logger.warn('Conexion perdida al crear Status/Result de tarea: ' + path);
                        that.statusCreate(path, data);
                        break;
                    case ZK_ERRORS.NODEEXISTS:
                        logger.warn('Status/Result de tarea ya existente: ' + path);
                        break;
                    default:
                        logger.error("Error al crear Status/Result de tarea: " + path);
                        break;
                    }
                }else{
                    logger.info('Status/Result de tarea creado exitosamente: ' + path);
                }
            });
    };
        ...
        that.zk.remove(path, -1 , function(error){
            if (error) {
                ...
                case ZK_ERRORS.CONNECTIONLOSS:
                    ...
                    break;
                default:
                    logger.error("Error al elminar la asignacion de tarea:: " + path);
                    console.dir(error);
                    break;
                }
            }else{
                logger.info('Asignacion de tarea eliminada exitosamente: ' + path);
            }
        });
    }
    ...
    this.init = function(callback){
        logger.debug("INICIANDO TRANSACTION WORKER");
        async.series({
